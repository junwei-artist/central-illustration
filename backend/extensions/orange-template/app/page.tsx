...
export default function Home() {
  ...
  // Helper to change slide and notify parent window (for editor integration)
  const changeSlide = (newSlide: number) => {
    setCurrentSlide(newSlide)
    // Notify parent window if we're in an iframe (editor preview)
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: 'SLIDE_CHANGED', slide: newSlide }, '*')
    }
  }
  ...
  // Discover how many pages exist by probing content files
  const discoverSlidesCount = async () => {
    const currentPath = window.location.pathname
    const exists = async (pageNum: number) => {
      const pageFolder = `page-${pageNum}`
      let url
      if (currentPath.startsWith('/proxy/')) {
        const proxyMatch = currentPath.match(/^\/proxy\/\d+/)
        url = proxyMatch ? `${window.location.origin}${proxyMatch[0]}/content/${pageFolder}/title.md` : `/content/${pageFolder}/title.md`
      } else {
        url = `/content/${pageFolder}/title.md`
      }
      try {
    ...
    let last = 0
    // Probe up to 50 pages
    for (let i = 1; i <= 50; i++) {
      // eslint-disable-next-line no-await-in-loop
      const ok = await exists(i)
      if (ok) last = i
      else break
    }
    setSlidesCount(Math.max(2, last))
  }
  ...
  useEffect(() => {
    ...
    // Check hash for initial slide/page (e.g., #slide=0&page=3)
    ...
    if (pageMatch) {
      ...
      if (!Number.isNaN(pageNum)) {
        loadPageContent(pageNum - 1)
      }
    }
    
    // Listen for postMessage to change slides (for editor integration)
    const handleMessage = (event: MessageEvent) => {
      if (event.data?.type === 'SET_SLIDE' && typeof event.data.slide === 'number') {
        changeSlide(event.data.slide)
      }
      if (event.data?.type === 'SET_PAGE' && typeof event.data.page === 'number') {
        const pageNum = event.data.page
        if (Number.isFinite(pageNum)) loadPageContent(pageNum - 1)
      }
      if (event.data?.type === 'CONTENT_UPDATED') {
        if (typeof event.data.page === 'number') {
          ...
          loadPageContent(currentSlide)
        }
      }
    }
    window.addEventListener('message', handleMessage)
    
    // Also respond to hash changes (#slide=N&page=M)
    const handleHashChange = () => {
      const hashNow = window.location.hash
      const slideMatch2 = hashNow.match(/slide=(\d+)/)
      if (slideMatch2) {
        const s = parseInt(slideMatch2[1], 10)
        if (!Number.isNaN(s)) setCurrentSlide(s)
      }
      const pageMatch2 = hashNow.match(/page=(\d+)/)
      if (pageMatch2) {
        const p = parseInt(pageMatch2[1], 10)
        if (!Number.isNaN(p)) loadPageContent(p)
      }
    }
  ...
  // Reload markdown when slide changes (keeps live preview in sync with editor)
  useEffect(() => {
    loadPageContent(currentSlide)
  }, [currentSlide])

  // Provide default slide props per index, alternating styles/colors
  const getSlideDefaults = (index: number) => {
    if (index % 2 === 0) {
      return {
    ...
    <div className="bg-gray-50">
      ...
      <section className="relative overflow-hidden h-screen">
        {[...Array(slidesCount)].map((_, index) => {
          const slide = getSlideDefaults(index)
          return (
          <motion.div
            key={index}
            initial={{ opacity: 0 }}
        ...
        {/* Previous/Next Buttons */}
            <button
          onClick={() => changeSlide((currentSlide - 1 + slidesCount) % slidesCount)}
          ...
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
          onClick={() => changeSlide((currentSlide + 1) % slidesCount)}
